...
editModPoly <- function(input, output, session, leafmap, targetLayerId = NULL, sf = TRUE, record = FALSE, crs = 4326) {
  ...
  if (is.null(Find(function(cl) {
    ...
    leafmap <- leaflet.extras::addDrawToolbar(leafmap,
      targetGroup = targetLayerId, polylineOptions = FALSE, polygonOptions = leaflet.extras::drawPolygonOptions(repeatMode = TRUE),
      circleOptions = FALSE, rectangleOptions = leaflet.extras::drawRectangleOptions(repeatMode = TRUE), markerOptions = FALSE, editOptions = leaflet.extras::editToolbarOptions()
    )
  }
  ...
  featurelist <- reactiveValues(drawn = list(), edited_all = list(), deleted_all = list(), finished = list())
  ...
  shiny::observeEvent(input[[EVT_EDIT]], {
    ...
    # find the edited features and update drawn start by getting the leaflet ids to do the match
  ...
  shiny::observeEvent(input[[EVT_DELETE]], {
    ...
    # find the deleted features and update finished start by getting the leaflet ids to do the match
    ids <- unlist(lapply(featurelist$finished, function(x) {
      x$properties$`_leaflet_id`
    }))
  ...
  if (record == TRUE) {
    lapply(c(EVT_DRAW, EVT_EDIT, EVT_DELETE), function(evt) {
      observeEvent(input[[evt]], {
        recorder <<- c(recorder, list(list(event = evt, timestamp = Sys.time(), feature = input[[evt]])))
      })
    })
  }


  # collect all of the the features into a list by action
  returnlist <- reactive({
    workinglist <- list(drawn = featurelist$drawn, edited = featurelist$edited_all, deleted = featurelist$deleted_all, finished = featurelist$finished)
    # if sf argument is TRUE then convert to simple features
    if (sf) {
      workinglist <- lapply(workinglist, function(action) {
        # ignore empty action types to prevent error handle in the helper functions?
        if (length(action) == 0) {
          return()
        }

        # FeatureCollection requires special treatment and we need to extract features
        ...
        combine_list_of_sf(lapply(features, st_as_sf_geo_list, crs = crs))
      })
