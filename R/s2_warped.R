#' @title Make a warped of each band of S2 tiles with the same date and orbit
...
s2_warped <- function(infiles,
  ...
  # We have 3 UTM zones over France. To merge them, we need to resample in common
  ...
  for (fic in infiles) {
    ...
    if (!file.exists(file.path(outdir, basename(fic)))) {
      # verify if *TILES.tif exist
      if (file.exists(file.path(tilesdir, gsub("_L93_WARPED_", "_TILES_", basename(fic))))) {
        cmd <- paste0(
          "parallel -j", cpu, " ",
          binpaths$gdalwarp, " -q -r cubic -t_srs 'EPSG:2154' -tr ",
          resolution, " ", resolution, " ",
          paste0(file.path(translatedir, paste0(gsub(
            "_L93_WARPED_", "_TRANSLATE_",
